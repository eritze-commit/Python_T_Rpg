...
def Item_use(Item_name):
    ...
    if Item_name in ("lhpouch","cmpouch","chpouch","mpouch","lmpouch","hpouch"):
        ...
        if Item_name == "morb":
            Number_anim(40,0.02,"+MANA",int(health/1.5),2)
        else:
            Number_anim(40,0.02,"+HEALTH",int(mana/1.5),2)
...
def Inventory():
    ...
    item_list = {
            ...
            "hpotog":"-\n[Health potion of greed] Restores Mana based on youre current gold \n-",
            ...
            "hwater":"-\n[Holy Water] Makes you a better person, burns if eliv, heals if holy \n-",
            "dpot":"-\n[Demon Potion] The more Evil you are the stronger the potion gets makes you more evil, healts either mana or health at random \n-",
            "lootp":"-\n[Loot pouch] Like a loot box but worse has a chance to drop good loot \n-",
...
def  Monster_F(name, Hmin, MaxD, Agro, atribute):                             #BATTLE ENGINE
    ...
    def status_effect():                                                               #STATUS EFFECT
        ...
        if atribute in ("PSN", "DOUBLE PSN") and Mhp > 0:
            if atribute == "PSN":
                ...
                Number_anim(30,0.02,"-HEALTH",10,2)
                Number_anim(30,0.02,"-HEALTH",10,2)
        if atribute == "LFS" and Mhp > 0 and random.randint(1,20) < 10:
            ...
            Mhp += Rdm
        if atribute == "GREEDY" and random.randint(1,20) < 15 :
            print(Fore.YELLOW + Back.WHITE+"ThE GoBliN LikEs GoulD")
        if atribute == "MGC" and Mhp > 0 and random.randint(1,20) < 15:
            print(Fore.WHITE + Back.BLUE +f"◀ ◀ ◀ captured mana")
            Number_anim(30,0.02,"-MANA",10,2)
        if atribute == "BRN" and Mhp > 0 and random.randint(1,20) < 15 :
    ...
    def palyer_atck():                                                   #PLAYER ATACK
        ...
        def damage_calc_spells():
            ...
            spellos = {
                "a":[random.randint(1+int((sc_vl*dif_vl)/(5+(sc_vl/1100))),20+int((sc_vl*dif_vl)/(3+(sc_vl/900)))),random.randint(1+int((sc_vl*dif_vl)/(25+(sc_vl/1100))),10+int((sc_vl*dif_vl)/(9+(sc_vl/1100)))),"Spell[A]","normal"],
                "b":[random.randint(int(mana/2),int(mana)),random.randint(int(mana/2),int(mana)),"Spell[B]","normal"]
            }
            while True:
        ...
        if spell_to_M[4] == True:
            if random.randint(1,100) < 6:
                ...
                print(Fore.RED + Back.BLACK + f"\rused {spell_to_M[2]} ◀CRIT▶ {name} -{int(spell_to_M[0]*1.5)} hp ◀CRIT▶")
            else:
                for i in range(30):
                    Rdm = random.randint(20+int((sc_vl*dif_vl)/(3+(sc_vl/1100))),50+int((sc_vl*dif_vl)/(1.2+(sc_vl/900))))
    ...
    def M_Used_Potion():                                       #MONSTER POTION USE 
        ...
        if (M_Pots >= 1 or E_Boss == True) and (random.randint(1,10) < 5 or E_Boss == True) and (Mhp < (Const_mhp/2) or E_Boss == True):
...
def merchant():
        ...
        prices = {
            "maxs": random.randint(5,5+int((sc_vl*dif_vl)/(16+(sc_vl/900)))),
            ...
            "invslot": random.randint(5,20+int((sc_vl*dif_vl)/(16+(sc_vl/900)))),
...
def Story_mini_game():
    def run():
        rand_dialuoge=["Are you going to run youre whole life..."]
        print()
    rand_actions = ["run","steal","key","look","back"]
